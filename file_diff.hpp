...
public:
    ...
    static auto compute_signature(const std::string& input_string, const std::size_t chunk_size) -> Signature
    {
    ...
    static auto compute_delta(const std::string& my_string, const Signature& signature, const std::size_t chunk_size)
        -> Delta
    {
    ...
    static auto apply_delta(const std::string& my_string, const Delta& delta, const std::size_t chunk_size)
        -> std::string
    {
    ...
    static auto split_into_chunks(const std::string& input_string, const std::size_t chunk_size)
...
private:
    ...
    static auto compute_rolling_hashes(const std::string& input, const std::size_t chunk_size) -> std::vector<Hash>
    {
        auto mod = static_cast<uint64_t>(1e9 + 7);
        auto base = 257; // A prime number
        auto base_powers = std::vector<uint64_t>(chunk_size + 1);
